...
``` bash
helm install mssql-deploy . --set ACCEPT_EULA.value=Y --set MSSQL_PID.value=Developer
```

After a few seconds this should deploy the SQL Server containers and you can see all the artifacts using the command :

```bash
D:\helm-charts\mssql\mssql>kubectl get all
```
